...
devices.  While the technical details of this platform are forthcoming as of
this paper's publication, it is reasonable to assume that the bridges will
incorporate a compatibility library to bridge the Android (or iOS) SDK and the
Windows~10 SDK.  In this paper, we will focus on native frameworks for
cross-platform mobile app development.  \figref{figure:xplatform-overview}
shows the typical workflow of app development using a native framework. We use
Xamarin as a concrete example, but the same general workflow applies to all
such native frameworks.}
