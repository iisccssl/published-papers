...
We now provide a high-level overview of the restricted space model. Our goal is
to motivate the use of some of the mechanisms that are described in
\sectref{section:mechanism}, and to describe our threat model.
...
of his devices during entry. Check-in can happen at a specially-designated
kiosk, where the host connects a guest device to its policy server to perform
the following tasks:
...
\code{/dev/kmem} interface in the end-user's work environment.  The host must
...
Note that the work environment may contain zero-day vulnerabilities, such as a
...
% MOVED FROM OLD INTRO:
% The portion of the monitor that resides in the secure world
% implements the \textit{mechanisms} to perform remote memory operations, while
% the remote portion of the monitor implements the \textit{policies}, and can be
% implemented by the enterprise on an in-house server, on the cloud, or in a USB
% dongle that can be attached to the employee's device to check its compliance. 

% Present the basic model assuming that the host is trusted.
% Guest is assumed to ``check in'' with the host.
% Show how guest would authenticate the host as it checks in.
% Host would check/send commands.
% Guest would possibly inspect these commands, and then accept it.
% At this point the guest is ``checked in''.
% Saved state of the guest is with the host for restoration when 
% the guest ``checks out''.

% FWD reference to possible attacks. 

% Talk in detail about RMO. talk about who performs the signing, and why the
% networking stack is in NW.

% How to prevent the guest from altering the state of the device?  Random nonce
% generated by the SW together with the device state.  Random nonce is saved in
% memory. Ability to replay the state of the device.  If device is powered off,
% cannot replay the state. Details of this goes into mechanism. Just mention that
